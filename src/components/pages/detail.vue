  ...
  <div class="detail">
    ...
    <group>
      <cell title="歌单" class="gedan" style="font-size:14px">
        ...
        <i slot="value" class="fa fa-search " aria-hidden="true" style="color: #fdfdfd;;font-size:25px;margin-right:15px;margin-left:15px"></i>
        <i after-title class="fa fa-ellipsis-v " aria-hidden="true" style="font-size:25px;color: #fdfdfd;"></i>
      </cell>
      <cell class="xia">
    ...
    <group >
      ...
      <cell v-for="(item,index) in ge" :key="index">
        <span slot="icon">{{index+1}}</span>
        <span slot="title">{{item.name}}</span>
        <span slot="inline-desc">{{item.ar[0].name}}-{{item.name}}</span>
        <i slot="value" style="    vertical-align:middle" class="fa fa-youtube-play" aria-hidden="true"></i>
      <i after-title class="fa fa-ellipsis-v " aria-hidden="true" style="font-size:18px;    vertical-align:middle" @click="tankuang(item)"></i>
      </cell>
    </group>
    </div>
      <div class="tankuang">
...
export default {
  ...
  created () {
      var that=this
      var url ="http://localhost:3000/user/detail?uid="+this.$store.state.Gendan.data.playlist.userId
